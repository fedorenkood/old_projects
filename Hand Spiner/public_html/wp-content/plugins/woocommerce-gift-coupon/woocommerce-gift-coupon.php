 ...
 * Description: This plugin generates coupons from products bought by WooCommerce, once generated customer sends PDF coupons by email
 * Depends: WooCommerce
 * Version: 2.3
...
define( 'WOOCOMMERCE_GIFT_COUPON_VERSION', '2.3' );
...
function woocommerce_gift_coupon_plugin_scripts() {
    ...
    wp_register_script( 'woocommerce_gift_coupon_js', WOOCOMMERCE_GIFT_COUPON_URL . 'admin/js/woocommerce-gift-coupon.js', array( 'jquery' ), false, true  );
    wp_enqueue_script( 'woocommerce_gift_coupon_js' );
}
...
////////////////////////////////////////////////////////////////////////////////
// WOOCOMMERCE - AUTOMATICALLY SEND
////////////////////////////////////////////////////////////////////////////////
add_action( 'woocommerce_order_status_completed', 'woocommerce_gift_coupon_automatically_send' );
...
function woocommerce_gift_coupon_send_action($post_ids){
	...
	require_once( WOOCOMMERCE_GIFT_COUPON_DIR . "lib/dompdf/dompdf_config.inc.php");
    }
        ...
        foreach ( $items as $key => $item ) {
            ...
            if( $sc == true ) {
                for ( $i = 1; $i <= $item["qty"]; $i++ ) {
                    ...
                    $coupons_mail['price'] = $amount[0];
                    $coupons_mail['symbol'] = get_woocommerce_currency_symbol();
                    ...
                    //Generate PDF
                    if(!empty( $body ) && class_exists( 'DOMPDF' ) ) { 
                        $attachment = woocommerce_gift_coupon_generate_pdf( $body );
                    }
...
function woocommerce_gift_coupon_generate_pdf( $body ) {
    $upload_dir = wp_upload_dir();
        ...
        if ( wp_mkdir_p( $pathupload ) && ! file_exists( trailingslashit( $pathupload ) . '.htaccess' ) ) {
            if ( $pdf_handle = @fopen( trailingslashit( $pathupload ) . '.htaccess', 'w' ) ) {
		fwrite( $pdf_handle, 'deny from all' );
		fclose( $pdf_handle );
            }
    }  
    $dompdf = new DOMPDF();
    ...
    $dompdf->render();
    $ficheropath = $pathupload.'/'.time().'.pdf';
...
////////////////////////////////////////////////////////////////////////////////
// WOOCOMMERCE - TABLE ALTER - COUPONS DATA
////////////////////////////////////////////////////////////////////////////////
...
function woocommerce_gift_coupon_render_columns( $column ) {
    ...
    switch ( $column ) {
        ...
        break;    
        case 'coupon_status' :
            $coupons_generated = $wpdb->get_var( "SELECT COUNT(*) FROM " . $wpdb->prefix . "woocommerce_gift_coupon WHERE id_order='" . $post->ID . "'" );
	...
	break;
        case 'coupons' :
            $coupons_generated = $wpdb->get_results( "SELECT * FROM " . $wpdb->prefix . "woocommerce_gift_coupon WHERE id_order='" . $post->ID . "'" );
