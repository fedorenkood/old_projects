...
if ( ! defined( 'ABSPATH' ) ) exit;
                ...
                if( ! empty($post_subscription_plans ) ) {
                    ...
                    foreach( $post_subscription_plans as $post_subscription_plan ) {
                        ...
                        foreach( $member_subscriptions as $member_subscription ) {
                            if( $post_subscription_plan == $member_subscription->subscription_plan_id && ( $member_subscription->status == 'active' || $member_subscription->status == 'canceled' ) ) {
    ...
    function pms_member_cancel_subscription( $content ) {
        ...
        $output = '<form id="pms-cancel-subscription-form" action="" method="POST" class="pms-form">';
            ...
            $output .= '<span id="cancelme" class="btn">Cancel Membership</span>';
            $output .= '<div id="cancelbtn" style="display:none">';
            ...
            // Output submit button
            $output .= '<input type="submit" class="btn" name="pms_confirm_cancel_subscription" value="' . apply_filters( 'pms_cancel_subscription_button_value', __( 'Confirm', 'paid-member-subscriptions' ) ) . '" />';
            $output .= '</div>';
    ...
    function pms_member_abandon_subscription( $content ) {
        ...
        $output .= '<span id="cancelme" class="btn">Cancel Membership</span>';
        $output .= '<div id="cancelbtn" style="display:none">';
        $output .= apply_filters( 'pms_abandon_subscription_confirmation_message', '<p>' . sprintf( __( 'Are you sure you want to cancel your %s subscription? This subscription will be removed completely from your account.', 'paid-member-subscriptions' ) . '</p>', $subscription_plan->name ), $subscription_plan );

        
        // Output submit button
        $output .= '<input type="submit" class="btn" name="pms_confirm_abandon_subscription" value="' . apply_filters( 'pms_abandon_subscription_button_value', __( 'Abandon Subscription', 'paid-member-subscriptions' ) ) . '" />';
        $output .= '</div>';
    ...
    function pms_member_retry_payment_subscription( $content ) {
        ...
        // Output submit button
        $output .= '<input type="submit" class="btn" name="pms_confirm_retry_payment_subscription" value="' . apply_filters( 'pms_retry_payment_subscription_button_value', __( 'Retry payment', 'paid-member-subscriptions' ) ) . '" />';
